...
In summary, we believe that the effort of using the G-formula (less than five lines of code in R or Stata) or doubly robust techniques is minimal (one line of code after loading a library [TMLE](https://cran.r-project.org/web/packages/tmle/index.html), [ELTMLE](https://github.com/migariane/eltmle)), and one has the advantage of not making any (possibly unrealistic) assumptions about effect modification and collapsibility. In our opinion, it does not matter if the “bias” is small or big, a method that avoids a certain type of bias should be preferred over one that does not. 
